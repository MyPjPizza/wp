...
function geodir_ipn_handler_paypal() {
	...
	if (!$fp) { 
		...
		while (!feof($fp)) {
            ...
            /// PM TO INVOICING IPN
            if ( !empty( $invoice_id ) && defined( 'WPINV_VERSION' ) ) {
                $wpi_invoice        = !empty( $invoice_info->invoice_id ) ? wpinv_get_invoice( $invoice_info->invoice_id ) : NULL;
                $wpi_invoice_id     = !empty( $wpi_invoice ) && !empty( $wpi_invoice->ID ) ? $wpi_invoice->ID : wpinv_cpt_save( $invoice_id );
                
                if ( $wpi_invoice_id ) {
                    $_POST['custom'] = $wpi_invoice_id;
                    
                    wpinv_process_paypal_ipn();
                    return;
                }
            }
            ///
...
function geodir_ipn_handler_2co() {
    ...
    if ( !empty( $invoice_id ) && defined( 'WPINV_VERSION' ) ) {
        ...
        if ( $wpi_invoice_id ) {
            $_POST['vendor_order_id'] = $wpi_invoice_id;
            
            wpinv_process_2co_ipn();
            return;
        }
    }
    ///
