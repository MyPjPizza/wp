...
function geodir_save_listing_package_fields($post_id='', $request_info) {
	...
	$saved_revision = get_option( 'geodir_revision_' . $post_id );
	$save_revision = !empty( $saved_revision[0] ) && is_array( $saved_revision[0] ) ? (array)$saved_revision[0] : (array)$post_info;
	...
	$current_revision = (array)geodir_get_post_info( $post_id );
		...
		foreach ($custom_fields as $key=>$val) {
			...
			if (!($field_type == 'address' && $is_admin == '1') && !($field_type == 'taxonomy' && $is_admin == '1') && $val['for_admin_use']!='1') {
				if (in_array($package_id,$packages)) { // if active for this package then dont change
					if ( isset($current_revision[$val['name']]) ) {
						$save_revision[$val['name']] = $current_revision[$val['name']];
					}
				} else { // if not active in this package then blank
					geodir_save_post_meta($post_id, $val['name'],'');
					
					$current_revision[$val['name']] = '';
				}
			}
		}
	}
	
	// Save listing package revision.
	$saved_revision[0] = $save_revision;
	$saved_revision[$package_id] = $current_revision;
	
	update_option( 'geodir_revision_' . $post_id, $saved_revision );
}
...
/**
 * Check and set the custom field revision value for requested package.
 *
 * @since 2.0.0
 * 
 * @global object $gd_session GeoDirectory Session object.
 ...
 * @return array Filtered custom field value.
 */
function geodir_payment_check_listing_revision( $value, $cf ) {
    global $gd_session, $post;
    ...
    if ( is_admin() ) {
        ...
        $req_pkg_id = !empty( $_REQUEST['package_id'] ) ? (int)$_REQUEST['package_id'] : 0;
    } else {
        if ( isset( $_REQUEST['backandedit'] ) && $_REQUEST['backandedit'] && $gd_ses_listing = $gd_session->get( 'listing' ) ) {
            return $value;
        }
        
        if ( !empty( $post->ID ) && !empty( $_REQUEST['pid'] ) && $post->ID == (int)$_REQUEST['pid'] ) {
            $post_id = (int)$_REQUEST['pid'];
            $req_pkg_id = !empty( $_REQUEST['package_id'] ) ? (int)$_REQUEST['package_id'] : 0;
        }
    }

    if ( !empty( $cf['name'] )  && $post_id > 0 && $req_pkg_id > 0  ) {
        $value = geodir_payment_get_field_revision( $value, $cf['name'], $post_id, $req_pkg_id );
    }
        
    return $value;
}
add_filter( 'geodir_get_cf_value', 'geodir_payment_check_listing_revision', 10, 2 );

/**
 * Get the custom field revision value for requested package.
 *
 * @since 2.0.0
 * 
 * @global array $gd_revisions Listing package revisions.
 * @global array $gd_current_revision Current listing package revision.
 *
 * @param mixed $value Custom field value.
 * @param string $field Field name.
 * @param int $post_id Post ID.
 * @param int $req_pkg_id Requested listing package ID.
 * @param int $saved_pkg_id Saved listing package ID. Default 0.
 * @return mixed Filtered custom field value.
 */
function geodir_payment_get_field_revision( $value, $field, $post_id, $req_pkg_id, $saved_pkg_id = 0 ) {
    global $gd_revisions, $gd_current_revision;
    
    if ( empty( $field ) || !$post_id > 0 || !$req_pkg_id > 0 ) {
        return $value;
    }
    
    if ( !$saved_pkg_id > 0 ) {
        if ( empty( $gd_current_revision ) ) {
            $gd_current_revision = (array)geodir_get_post_info( $post_id );
        }
    
        $saved_pkg_id = !empty( $gd_current_revision['package_id'] ) ? $gd_current_revision['package_id'] : 0;
    }
    
    if ( !( $saved_pkg_id > 0 && $saved_pkg_id != $req_pkg_id ) ) {
        return $value;
    }
    
    $current_pkg_fields = geodir_payment_package_fields( $saved_pkg_id, get_post_type( $post_id ) );
    
    if ( !empty( $current_pkg_fields ) && !in_array( $field, $current_pkg_fields ) ) {
        if ( !empty($gd_revisions) && isset( $gd_revisions[$post_id] ) ) {
            $saved_revision = $gd_revisions[$post_id];
        } else  {
            $saved_revision = get_option( 'geodir_revision_' . $post_id );
            $gd_revisions[$post_id] = $saved_revision;
        }
        
        $revision_value = $value;
        if ( !empty( $saved_revision[0] ) && isset( $saved_revision[0][$field] ) ) {
            $revision_value = $saved_revision[0][$field];
        } else if ( !empty( $saved_revision[$req_pkg_id] ) && isset( $saved_revision[$req_pkg_id][$field] ) ) {
            $revision_value = $saved_revision[$req_pkg_id][$field];
        }
        
        $value = $revision_value && $revision_value !== $value ? maybe_serialize( $revision_value ) : $revision_value;
    }
