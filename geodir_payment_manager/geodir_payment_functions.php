...
function geodir_expire_check() {
    global $wpdb, $plugin_prefix, $gd_skip_preexpiry, $gd_posts_to_expire;
        ...
        foreach ($geodir_postypes as $post_type) {
            ...
            if (geodir_payment_preexpiry_notice_is_active() && !$gd_skip_preexpiry) {
                ...
                if (!empty($rows)) {
                    foreach ($rows as $row) {
                        ...
                        if (empty($package_info->recurring_pkg)) {
                            update_post_meta($row->ID, '_gdpm_recurring', false);
                        }
                        
                        if (get_post_meta($row->ID, '_gdpm_recurring', true)) { // Don't send expiration for recurring listing
                            continue;
                        }
            ...
            if (!empty($postid_str)) {
                foreach ($postid_str as $postid_str_obj) {
                    ...
                    if (empty($old_package_info->recurring_pkg)) {
                        update_post_meta($post_id, '_gdpm_recurring', false);
                    }
                    
                    if (get_post_meta($post_id, '_gdpm_recurring', true)) { // Don't expire recurring listing
                        continue;
                    }
