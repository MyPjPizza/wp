...
class GP_Post_Content_Merge_Tags extends GWPerk {
    ...
    public function init() {
        ...
        add_filter( 'gform_entry_created',      array( $this, 'save_pretty_id' ), 20, 3 );
	...
	public function tiny_mce_init( $settings ) {
		...
		if( rgars( $settings, 'content/selector' ) == '#content' ): ?>
			...
			<?php if( ! wp_script_is( 'gform_form_admin' ) ):
				wp_print_scripts( array( 'gform_gravityforms', 'gform_form_admin' ) );
			endif; ?>
    ...
    function maybe_decrypt_entry_id( $entry_id ) {
        ...
        if( ! $entry_id ) {
            ...
            if( ! $decrypted_entry_id && $this->is_pretty_id_enabled() ) {
            	return $entry_id;
            }
    ...
    function add_confirmation_url_entry_detail( $form_id, $entry ) {
		...
		if ( $gppcmt_url ) {
	        ...
	        printf(
		        '%1$s: <a href="%2$s" alt="%3$s" title="%3$s">%4$s</a>',
