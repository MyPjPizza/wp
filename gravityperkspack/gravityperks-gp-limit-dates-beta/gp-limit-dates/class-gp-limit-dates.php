...
class GP_Limit_Dates extends GWPerk {
	...
	public function register_scripts() {
		...
		wp_register_script( 'gp-limit-dates', $this->get_base_url() . '/js/gp-limit-dates.js', array( 'gform_datepicker_init' ), $this->version );
		...
		<script type="text/javascript">
			...
			( function( $ ) {
				...
				function fieldSettingsInit( field ) {
					...
					$minDateSelect.val( field[ key( 'minDate' ) ] ).change();
					...
					$maxDateSelect.val( field[ key( 'maxDate' ) ] ).change();
	...
	public function get_limit_dates_options( $form_or_field, &$options = array() ) {
		...
		if( isset( $form_or_field['fields'] ) ) {
			 ...
			 */
			return gf_apply_filters( 'gpld_limit_dates_options', array( $form['id'] ), $options, $form, null );
			...
			$days_of_week = $field->{$this->key( 'daysOfWeek')};
			$has_days_of_week = count( $days_of_week ) > 0 && count( $days_of_week ) < 7;
			...
			$field_options = array(
				...
				'daysOfWeek' => $has_days_of_week ? $days_of_week : false,
	...
	public function has_limit_dates_enabled( $form_or_field ) {
		...
		if( is_a( $form_or_field, 'GF_Field' ) ) {
			...
			if( $field->get_input_type() != 'date' || $field->dateType != 'datepicker' ) {
				...
				$limit_date_options = $this->get_limit_dates_options( $field );
	...
	public function is_valid_date( $date, $field ) {
		...
		$is_valid         = $is_date_in_range && $is_valid_day;
		$is_excepted      = $this->is_excepted( $date, $field );
	...
	public function is_valid_day_of_week( $date, $field ) {
		...
		$days_of_week = rgar( $this->get_limit_dates_options( $field ), 'daysOfWeek' );
		if( ! $days_of_week ) {
			return true;
		}
	...
	public function is_excepted( $date, $field ) {
		...
		$exceptions = rgar( $this->get_limit_dates_options( $field ), 'exceptions' );
		if( empty( $exceptions ) ) {
			return false;
		}
	...
	public function get_date_value( $field, $key ) {
		...
		$options     = $this->get_limit_dates_options( $field );
