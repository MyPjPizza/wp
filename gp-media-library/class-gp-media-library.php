...
class GP_Media_Library extends GWPerk {
	...
	public function replace_image_merge_tags( $text, $form, $entry ) {
        ...
        foreach ( $matches as $match ) {
            ...
            if ( ! isset( $image_ids[ $input_id ] ) ) {
                continue;
            }

            foreach( $image_ids[ $input_id ] as $image_id ) {
                $_replace  = GFCommon::replace_variables_post_image( $search, array( $input_id => $image_id ), $entry );
	            if( in_array( 'link', $match ) ) {
            ...
            $text = str_replace( $search, implode( "\n", $replace ), $text );
