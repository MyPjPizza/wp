...
class PUM_ATC_Migration {
	...
	public static function process_updates() {
		...
		if ( ! get_option( 'pum_atc_db_ver', false ) || get_option( 'pum_atc_db_ver', false ) < PUM_ATC::$DB_VER ) {

			$v  = PUM_ATC::$DB_VER;
			$cv = get_option( 'pum_atc_db_ver', false );
	...
	public static function run( $version = null ) {
		...
		if ( ! method_exists( __CLASS__, 'v' . $version . 'migration' ) ) {
			return;
		}
		...
		call_user_func( array( __CLASS__, 'v' . $version . 'migration' ) );

		if ( method_exists( __CLASS__, 'v' . $version . 'cleanup' ) ) {
			call_user_func( array( __CLASS__, 'v' . $version . 'cleanup' ) );
		}

		update_option( 'pum_atc_db_ver', $version );
	}

	/**
	 * DB v2 Migration
	 */
	public static function v2migration() {
		...
		foreach ( $popups as $popup ) {
			PUM_ATC_Migration::v2conditions( $popup->ID );
		}
	 ...
	 * @param $popup_id
	 */
	public static function v2conditions( $popup_id ) {
		$popup          = new PUM_Popup( $popup_id );
		$conditions     = $popup->get_conditions();
		$old_conditions = PUM_ATC_Migration::get_v1_conditions( $popup_id );
	 ...
	 * @return array|mixed
	 */
	public static function get_v1_conditions( $popup_id = 0 ) {
