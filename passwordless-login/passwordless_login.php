...
function wpa_front_end_login(){
	...
	if( $account && !is_wp_error($sent_link) ){
		echo '<p class="wpa-box wpa-success">'. apply_filters('wpa_success_link_msg', __('Cheeck your email for your Magic Link.', 'passwordless') ) .'</p>';
	...
	<form name="wpaloginform" id="wpaloginform" action="" method="post">
		<p>
			...
			<input type="submit" name="wpa-submit" id="wpa-submit" class="button-primary" value="<?php esc_attr_e('Magic Link'); ?>" />
		</p>
...
function wpa_valid_account( $account ){
	...
	return new WP_Error( 'invalid_account', __( "Sorry, that email isn't associated with an account. Try again or register again.", "passwordless" ) );
}
...
function wpa_send_link( $email_account = false, $nonce = false ){
    ...
    $headers[] = 'From: Geotourism Indonesia <cs@geotourism.id>';
		$unique_url = wpa_generate_url( $valid_email , $nonce );
		$subject = apply_filters('wpa_email_subject', __("MAGIC LINK for $blog_name"));
		$message = apply_filters('wpa_email_message', __('<h2>'.$blog_name.'</h2><p>Hello!</p><br><p>Use this MAGIC LINK to login at '.$blog_name.'<p>Visit:</p> <a href="'. esc_url( $unique_url ) .'" target="_blank">'. esc_url( $unique_url ) .'</a><br><p>Sincerely,</p><p>The Team @ Geotourism Indonesia</p><br>https://wwww.geotourism.id<br>https://facebook.com/geotourism.id'), $unique_url, $valid_email);
